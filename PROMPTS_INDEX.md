# Prompts Index

Quick reference for the repository layout and current content.

## Root

- `README.md` - repository overview and structure rules
- `PROMPTS_INDEX.md` - compact content index

## docs/

- `docs/repo/improvement-prompt.md` - repository reorganization prompt
- `docs/company/myrentalspot/modernization-migration-plan.md` - MyRentalSpot modernization plan
- `docs/company/myrentalspot/project-technical-documentation.md` - MyRentalSpot technical documentation
- `docs/exams/rajasthan-basic-computer-instructor/rajasthan-bci-exam-analysis-research-report.md` - Rajasthan BCI exam analysis research report

## templates/

- `templates/messages/sample-laravel-job-application-whatsapp.md` - sample Laravel job application WhatsApp message

## prompts/

- `prompts/general/interview/screening-round.md` - general screening interview prompt
- `prompts/general/interview/laravel-rate-limiting-guide.md` - Laravel rate limiting guide
- `prompts/laravel/interview/laravel-php-backend-interview-qna.md` - Laravel and PHP backend interview Q&A prompt
- `prompts/laravel/rbac-dynamic-acl-laravel-vue-inertia.md` - RBAC and dynamic ACL implementation (Laravel, Vue, Inertia)
- `prompts/laravel/tools/laravel-interview-qna-generator.md` - Laravel interview Q&A generator
- `prompts/react/react-interview-prep-prompt.md` - React interview preparation prompt
- `prompts/node/learning/nodejs-learning-roadmap.md` - Node.js learning roadmap
- `prompts/vue/interview/laravel-vue-inertia-interview-guide.md` - Laravel Vue Inertia interview guide
- `prompts/vue/interview/vue-interview-prep-prompt.md` - Vue.js interview preparation prompt
- `prompts/exams/rajasthan-basic-computer-instructor/database-management-system.md` - Rajasthan BCI DBMS and SQL study pack prompt
- `prompts/exams/rajasthan-basic-computer-instructor/computer-fundamentals.md` - Rajasthan BCI computer fundamentals study pack prompt
- `prompts/exams/rajasthan-basic-computer-instructor/exam-analysis-and-study-plan.md` - Rajasthan BCI exam analysis and study strategy prompt
- `prompts/company/dotsquare/multi-round-laravel-interview.md` - DotSquare multi-round Laravel interview prompt
- `prompts/company/webtechno/project-analyzer.md` - WebTechno project analyzer prompt
- `prompts/company/webtechno/legacy-calendar-event-php-to-filament-v5-migration.md` - legacy calendar event PHP forms to Filament v5 migration prompt
- `prompts/company/webtechno/fix-filament-advanced-tables-trait-collision.md` - WebTechno Filament trait collision fix prompt
- `prompts/company/webtechno/resolve-all-advanced-tables-trait-collisions.md` - WebTechno full AdvancedTables collision resolution prompt
- `prompts/company/webtechno/fix-duplicate-relation-in-property.md` - WebTechno duplicate relation fix prompt
- `prompts/company/webtechno/saturio-migration-verification.md` - WebTechno migration verification prompt
- `prompts/company/webtechno/saturio-migration-verification-v2.md` - WebTechno migration verification v2 prompt
- `prompts/company/webtechno/saturio-email-todo-cron-audit.md` - WebTechno email headers, TODO cleanup, and cron audit prompt
- `prompts/company/webtechno/verification-property-valuation.md` - WebTechno valuation verification prompt
- `prompts/company/webtechno/database-snippets/query-01.sql` - database snippet 1
- `prompts/company/webtechno/database-snippets/query-02.sql` - database snippet 2
- `prompts/company/webtechno/database-snippets/query-03.sql` - database snippet 3
- `prompts/company/webtechno/database-snippets/query-04.sql` - database snippet 4
- `prompts/company/webtechno/database-snippets/query-05.php` - PHP database snippet

## interview-prep/

- `interview-prep/general/hr-interview.md` - HR interview questions and answers
- `interview-prep/laravel/laravel-interview-questions-and-answers.md` - core Laravel interview Q&A
- `interview-prep/laravel/screening-qna.md` - Laravel screening Q&A
- `interview-prep/laravel/core-concepts-qna.md` - Laravel core concepts Q&A
- `interview-prep/laravel/senior-qna.md` - Laravel senior-level Q&A
- `interview-prep/company/dotsquare/dotsquare-laravel-interview-qna.md` - DotSquare Laravel interview Q&A
- `interview-prep/company/square/round-1-basic-php-and-laravel-fundamentals.md` - Square round 1 guide
- `interview-prep/company/square/round-2-advanced-laravel-concepts.md` - Square round 2 guide
- `interview-prep/company/square/round-3-api-design-and-backend-architecture.md` - Square round 3 guide
- `interview-prep/company/square/round-4-database-and-query-optimization.md` - Square round 4 guide
- `interview-prep/company/square/round-5-stripe-payment-integration.md` - Square round 5 guide
- `interview-prep/company/square/round-6-aws-deployment-and-scaling.md` - Square round 6 guide
- `interview-prep/company/square/round-7-system-design-and-real-world-scenarios.md` - Square round 7 guide
- `interview-prep/company/square/round-8-debugging-and-problem-solving.md` - Square round 8 guide
- `interview-prep/company/square/interview-rounds-quick-reference.md` - Square quick reference summary
